...
var portfolioTabsNav = document.querySelector('.tabs__nav');
var portfolioTabsBtns = document.querySelectorAll('.tabs__nav-btn');
var portfolioTabsItems = document.querySelectorAll('.tabs__panel');
var portfolioTabsItemsVisible = document.querySelectorAll('.tabs__panel--visible');
var loadMore = document.querySelector('.portfolio__btn');
var maxItems = 3;
...
if (portfolioTabsNav) {
  ...
  var hideMoreItems = function hideMoreItems(selector) {
    if (selector.length > maxItems) {
      ...
      hiddenItems.forEach(function (el) {
        el.classList.remove('tabs__panel--visible');
        el.classList.remove('tabs__panel--visible-more');
      });
    }
  };
  ...
  portfolioTabsNav.addEventListener('click', function (e) {
    ...
    if (target.classList.contains('tabs__nav-btn')) {
      ...
      document.querySelectorAll("[data-target=\"".concat(path, "\"]")).forEach(function (el) {
        el.closest('.tabs__panel').classList.add('tabs__panel--visible');
      });
      isLoadMoreNeeded(document.querySelectorAll("[data-target=\"".concat(path, "\"]")));
      hideMoreItems(document.querySelectorAll('.tabs__panel--visible'));

      if (path == 'all') {
        portfolioTabsItems.forEach(function (el) {
          el.classList.add('tabs__panel--visible');
        });
        isLoadMoreNeeded(document.querySelectorAll('.tabs__panel--visible'));
        hideMoreItems(document.querySelectorAll('.tabs__panel--visible'));
      }
    }
  });
  ...
  loadMore.addEventListener('click', function (e) {
    var visibleItems = document.querySelectorAll('.tabs__panel--visible');
    var path = document.querySelector('.tabs__nav-btn').dataset.path;
    console.log(path);

    if (path == 'all') {
      portfolioTabsItems.forEach(function (el) {
        el.classList.add('tabs__panel--visible-more');
        loadMore.style.display = 'none';
      });
    } else {
      document.querySelectorAll("[data-target=\"".concat(path, "\"]")).forEach(function (el) {
        el.closest('.tabs__panel').classList.add('tabs__panel--visible-more');
      });
      loadMore.style.display = 'none';
    }
  });
